...
| `raw-access-event.schema.json` creado y aceptado sin `support_operator`; conserva la clase heredada de `TraceObject`/`RetrievalRun` | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/contracts/raw-access-event.schema.json`, `docs/policies/privacy-security-policy.md`, `docs/policies/trace-visibility-policy.md` |
| `prompt-injection-risk.schema.json` creado como contrato compartido y los riesgos `rr_*` conservan la clasificacion heredada del run | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/contracts/prompt-injection-risk.schema.json`, `docs/policies/prompt-injection-policy.md`, `docs/quality/initial-golden-dataset-spec.md` |
...
| `LegalSearchQuery`, `LegalEntity`, `RetrievalPlan` y `LegalSearchResult` tienen ownership e IDs canonicos; query, plan y run resuelven el mismo tenant/query | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/architecture/domain-model.md`, `docs/architecture/entity-ownership-matrix.md`, `docs/contracts/legal-search-query.schema.json`, `docs/contracts/retrieval-plan.schema.json`, `docs/contracts/retrieval-run.schema.json` |
...
| Claims juridicos criticos no pueden degradar criticality ni quedar fuera del grafo visible; todo claim final publicado esta verificado como `passed` y el golden dataset define oracle semantico independiente | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/contracts/answer-contract.schema.json`, `docs/contracts/claim.schema.json`, `docs/policies/citation-policy.md`, `docs/quality/initial-golden-dataset-spec.md` |
| `EvidencePack` respeta `RetrievalRun` opcional 1:1, usa refs reciprocas cuando existe run, reutiliza `EvidenceQuality` y alinea razones de rechazo entre `RetrievalRun` y `TraceObject` | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/contracts/evidence-pack.schema.json`, `docs/contracts/retrieval-run.schema.json`, `docs/contracts/trace-object.schema.json`, `docs/contracts/evidence-quality.schema.json`, `docs/architecture/domain-model.md` |
| API draft define upload multipart fail-closed, deshabilitado hasta fijar limite/allowlist versionados, y variantes de respuesta bloqueada/abstenida sin refs de evidencia inventadas | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/architecture/api-draft-v0.md`, `docs/policies/privacy-security-policy.md`, `docs/handoff/open-questions.md` |
...
| ADR-005 queda ejecutable mediante `PromptVersionRegistry` interno; toda `ModelCall.prompt_version` resuelve contra version/path/hash inmutables y solo `active` admite uso nuevo | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/adr/ADR-005-ai-provider-and-model-policy.md`, `docs/handoff/phase-1-implementation-brief.md`, `docs/handoff/sprint-1-backlog.md`, `docs/phases/phase-1-development-plan.md` |
| `validity_awareness` fija unidades atomicas desde las fuentes que el oracle exige materializar; una omision no reduce el denominador y `NO_APLICA` no diluye el score ni exige fuentes runtime ficticias | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/quality/evaluation-plan-v0.md`, `docs/quality/initial-golden-dataset-spec.md` |
| `ProviderCallAudit.trace_id` hereda tenant de un `TraceObject` materializado, nunca de un operational run sin traza final, y tiene ejemplo negativo cross-contract | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/contracts/provider-call-audit.schema.json`, `docs/architecture/domain-model.md`, `docs/policies/provider-policy.md`, `docs/handoff/phase-1-implementation-brief.md` |
| Canonicalizacion JSON contractual usa un unico helper JCS RFC 8785 con dependencia fijada, parser de claves unicas y tests de vectores/numeros | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/handoff/phase-1-implementation-brief.md`, `docs/handoff/sprint-1-backlog.md`, `docs/phases/phase-1-development-plan.md` |
| YAML canonicos usan un loader UTF-8 unico y estricto que rechaza claves duplicadas, tags, anchors, aliases, merge keys, documentos multiples, campos desconocidos y refs invalidas | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/schemas/README.md`, `docs/handoff/phase-1-implementation-brief.md`, `docs/handoff/sprint-1-backlog.md`, `docs/phases/phase-1-development-plan.md` |
| Error mapping de provider separa codigos internos de `ProviderCallAudit` de codigos publicos de `ErrorEnvelope` mediante traduccion cerrada | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/schemas/provider-registry.yaml`, `docs/contracts/provider-interfaces.md`, `docs/policies/provider-policy.md` |
| `OperationalRun` permite fallo temprano sin inventar inicio, cierra transiciones/timestamps y hace unicos los eventos singulares y terminales por run | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/handoff/phase-1-implementation-brief.md`, `docs/handoff/sprint-1-backlog.md`, `docs/phases/phase-1-development-plan.md` |
| Research credits respetan ventana de vigencia y vencimiento, debitos atomicos y saldo no negativo sin consumir grants expirados | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/handoff/phase-1-implementation-brief.md`, `docs/handoff/sprint-1-backlog.md`, `docs/phases/phase-1-development-plan.md` |
| Fetch boundary bloquea SSRF/DNS rebinding, proxies de ambiente y redirects automaticos; URLs rechazadas por acceso solo conservan hash canonico | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/contracts/provider-interfaces.md`, `docs/contracts/retrieval-run.schema.json`, `docs/policies/legal-search-policy.md`, `docs/handoff/risk-register.md` |
...
| Publicacion terminal, usage, creditos y cierre del run forman una unidad atomica; los artefactos comparten budget efectivo y retries/concurrencia no duplican consumo | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/policies/cost-governor-policy.md`, `docs/handoff/phase-1-implementation-brief.md`, `docs/handoff/sprint-1-backlog.md`, `docs/phases/phase-1-development-plan.md` |
| El endpoint de mensajes exige idempotencia tenant/actor-scoped por key hasheada/fingerprint y no duplica mensaje, run, usage o creditos ante retries concurrentes | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/architecture/api-draft-v0.md`, `docs/policies/privacy-security-policy.md`, `docs/handoff/phase-1-implementation-brief.md` |
| `TraceObject` y `RetrievalRun` distinguen contenido sanitizado de metadata-only, heredan la mayor sensibilidad aplicable y la conservan en `RawAccessEvent`; warnings/notas y snippets quedan acotados y sin controles, y todos los patterns de valor completo rechazan terminadores de linea mediante cierre real | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/contracts/README.md`, `docs/contracts/retrieval-run.schema.json`, `docs/contracts/legal-search-result.schema.json`, `docs/contracts/raw-access-event.schema.json`, `docs/architecture/domain-model.md`, `docs/architecture/entity-ownership-matrix.md`, `docs/policies/legal-search-policy.md`, `docs/policies/privacy-security-policy.md`, `docs/policies/trace-visibility-policy.md` |
| Todos los strings variables rechazan vacio y tienen `minLength: 1`/`maxLength`; arrays y transportes habilitados tienen cotas explicitas; API/persistencia aplican limites iguales o menores, DTOs publicos son cerrados/paginados y uploads permanecen fail-closed hasta fijar su limite | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/contracts/README.md`, `docs/architecture/api-draft-v0.md`, `docs/policies/privacy-security-policy.md`, `docs/handoff/phase-1-implementation-brief.md` |
...
| Orden de Fase 1 esta definido sin contratos, rutas o enums paralelos; usa `response-modes.yaml` y exige la regression suite inicial de ADR-012 | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/handoff/phase-1-implementation-brief.md`, `docs/handoff/sprint-1-backlog.md`, `docs/phases/phase-1-development-plan.md`, `docs/adr/ADR-012-evaluation-and-quality-gates.md` |
| `EvidencePack` cierra identidad local: areas sin duplicados, `source_ref`/`passage_ref` unicos y cada pasaje resuelve una fuente del mismo pack | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/contracts/evidence-pack.schema.json`, `docs/handoff/phase-1-implementation-brief.md`, `docs/handoff/sprint-1-backlog.md` |
| `Answer.latest_answer_version_ref` resuelve a la version vigente del mismo answer/tenant y conserva `response_outcome` consistente | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/architecture/domain-model.md`, `docs/policies/answer-versioning-policy.md`, `docs/handoff/phase-1-implementation-brief.md`, `docs/phases/phase-1-development-plan.md` |
| `Claim <-> Citation` queda como relacion n:m bidireccional y same-version, sin `claim_id`, `citation_ref` ni `sources_used[]` duplicados; citas distintas pueden compartir fuente y ninguna cita contractual queda huerfana | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/contracts/claim.schema.json`, `docs/contracts/citation.schema.json`, `docs/policies/citation-policy.md` |
| `CitationAudit.results[]` resuelve citas de la misma version con pares claim/citation unicos, fuente/pasajes exactos y relacion bidireccional comprobada; el fixture de Fase 1 hereda esas reglas sin adelantar el auditor productivo | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/policies/citation-policy.md`, `docs/policies/answer-versioning-policy.md`, `docs/handoff/phase-1-implementation-brief.md`, `docs/handoff/sprint-1-backlog.md`, `docs/phases/phase-1-development-plan.md` |
| `LegalSearchResult` exige razon cerrada cuando hay pasajes extraidos y el snapshot esta ausente o es `null`, mediante una condicion Draft 2020-12 explicita | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/contracts/legal-search-result.schema.json`, `docs/policies/legal-search-policy.md` |
| `Source.metadata` queda cerrado y minimizado, sin superficie para raw content o payloads de provider | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/contracts/source.schema.json`, `docs/policies/source-policy.md`, `docs/policies/privacy-security-policy.md` |
| Indice contractual enumera los 29 JSON Schemas aceptados sin omisiones | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/contracts/README.md`, `docs/contracts/` |
| Policies aceptadas declaran alcance y las criticas separan controles deterministas de asistencia IA, con revision operativa explicita | Si | Accepted | Codex / JusNova Chief Backend Architect | `docs/policies/README.md`, `docs/policies/non-negotiable-principles.md`, `docs/policies/provider-policy.md`, `docs/policies/legal-search-policy.md`, `docs/policies/privacy-security-policy.md`, `docs/policies/prompt-injection-policy.md`, `docs/policies/source-routing-matrix.md` |
