...
| LegalSearchResult | `(retrieval_run_id, lsr_*)` | Si | `organization_id` de `RetrievalRun` | Heredada de source/result; `USER_DOCUMENT` hereda confidencialidad documental | No aplica | Retener/tombstone con retrieval run; snapshots privados siguen politica documental | Resultado normalizado embebido; no es evidencia citable ni raw-access resource independiente en 0.11. |
