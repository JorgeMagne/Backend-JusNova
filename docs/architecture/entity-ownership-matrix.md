...
| LegalSearchQuery | `q_*` | Si | `organization_id` | `PUBLIC_LEGAL_QUERY` o `DERIVED_LEGAL_QUERY_RESTRICTED` | No aplica | Retener/tombstone con retrieval y trazas asociadas | Incluso una query publica abstracta pertenece al tenant que la ejecuto; no se reutiliza como global sin sanitizacion explicita. |
| LegalEntity | `(query_id, ent_*)` | Si | `organization_id` de `LegalSearchQuery` | Heredada de `LegalSearchQuery` | No aplica | Retener/tombstone con query | Value object detectado/resuelto; no crear catalogo global desde valores de usuario. |
| RetrievalPlan | `rp_*` | Si | `organization_id` | Heredada de `LegalSearchQuery` y `search_queries` | No aplica | Retener/tombstone con query/run | Plan de busqueda puede revelar estrategia o hechos del cliente; comparte tenant con query. |
| LegalSearchResult | `(retrieval_run_id, lsr_*)` | Si | `organization_id` de `RetrievalRun` | Heredada de source/result: fuente publica -> `PUBLIC_LEGAL_SOURCE`; resultado de documento usuario -> `USER_DOCUMENT_CONFIDENTIAL` o `DOCUMENT_EVIDENCE_CONFIDENTIAL` segun granularidad | No aplica | Retener/tombstone con retrieval run; snapshots privados siguen politica documental | Resultado normalizado embebido; no es evidencia citable ni raw-access resource independiente en 0.11. |
...
| RetrievalRun | `rr_*` | Si | `organization_id` | Hereda la mayor sensibilidad de query y resultados; minimo `INTERNAL_TRACE_RESTRICTED` | `retrieval_run` | Retener segun trazabilidad | Puede contener resultados normalizados y minimizados autorizados, incluidos snippets; `RawAccessEvent.classification` debe igualar su clase efectiva. Nunca conserva la respuesta raw del proveedor ni se embebe completo en `TraceObject`. |
...
| AbstentionRender | `abstention_render_*` | Si | `organization_id` | Hereda de `AnswerVersion`; minimo `INTERNAL_TRACE_RESTRICTED` | No aplica | Retener con answer version | Value object de abstencion/bloqueo; no contiene respuesta completa ni material raw. |
...
| TraceObject | `tr_*` | Si | `organization_id` | Hereda la mayor sensibilidad de mensajes, claims, evidencia, retrieval y riesgos; minimo `INTERNAL_TRACE_RESTRICTED` | `trace` | Retener por auditoria; no raw content | Solo existe al finalizar respuesta/abstencion/bloqueo; puede contener `Claim.text` derivado y `RawAccessEvent.classification` debe igualar su clase efectiva. |
...
Los summaries de retrieval, `CitationAudit`, `ModelCall` y `ToolCall` mantienen clasificacion `INTERNAL_TRACE_RESTRICTED` porque sus contratos aceptados solo permiten refs, hashes, contadores, codigos cerrados y metadata minimizada. `TraceObject` hereda la mayor clasificacion de sus mensajes, claims, evidencia, retrieval y riesgos porque puede contener `Claim.text` derivado; `RetrievalRun` hereda la mayor clasificacion de su query y resultados porque puede conservar resultados normalizados y minimizados. Ninguno autoriza persistir mensajes, documentos, OCR, HTML, prompts, model outputs ni provider payloads completos.
