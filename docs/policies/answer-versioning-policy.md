...
28. La canonicalizacion de `AnswerContract` usa el objeto completo validado por `answer-contract.schema.json`, incluyendo `organization_id`, `trace_id`, `answer_id`, `answer_version`, `evidence_pack_id`, `jurisdiction`, `sections` con `title` y `content`, `claims`, `citations`, `sources_used`, `warnings` y `created_at`; se serializa como JSON deterministico UTF-8 con claves de objeto ordenadas, arrays en el orden persistido y sin whitespace semantico.
