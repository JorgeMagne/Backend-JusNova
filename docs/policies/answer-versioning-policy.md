...
25. Si `response_outcome` es `answered` o `partial_abstention`, cada `AnswerContract.citations[]` debe estar cubierta por `TraceObject.citation_audit.results[]` con el mismo `citation_ref`, `passage_ref`, `source_ref`, `status = valid` y claims soportados equivalentes. Todo resultado que reutilice esa `citation_ref` debe resolver la misma cita y conservar exactamente su pasaje/fuente; no se permiten rutas de evidencia alternativas bajo el mismo ref.
26. Para cada cita publicada, `AnswerContract.citations[].status` debe ser `valid` y el conjunto `supports_claim_ids[]` debe coincidir con los `claim_id` auditados en `TraceObject.citation_audit.results[]` para la misma cita/pasaje/fuente. Cada par `(claim_id, citation_ref)` aparece una sola vez y existe en ambos sentidos de `Claim <-> Citation`.
...
28. La canonicalizacion de `AnswerContract` usa el objeto completo validado por `answer-contract.schema.json`, incluyendo `organization_id`, `trace_id`, `answer_id`, `answer_version`, `evidence_pack_id`, `jurisdiction`, `sections` con `title` y `content`, `claims`, `citations`, `sources_used`, `warnings` y `created_at`. `canonicalize` es JSON Canonicalization Scheme (JCS, RFC 8785) sobre el valor JSON ya validado, codificado como UTF-8 sin BOM: JCS fija orden de propiedades, serializacion de numeros y escapes de strings; los arrays conservan el orden persistido y no se aplica normalizacion Unicode adicional. El parser debe rechazar nombres de propiedad duplicados antes de validar o persistir el valor.
...
33. Para abstenciones y bloqueos, `AnswerVersion.answer_hash` debe coincidir con `AbstentionRender.render_hash`; ese hash cubre el cuerpo canonico renderizado almacenado bajo `AbstentionRender.render_storage_ref`. `canonicalize(render_body_canonical)` usa exactamente JCS (RFC 8785) y las precondiciones de la regla 28; `render_hash` es `sha256:` seguido del digest hexadecimal minusculo de los bytes UTF-8 resultantes. El cuerpo resuelto es un objeto cerrado `{ "content": <string> }`; `content` debe ser byte a byte el mismo valor persistido en el `Message` resuelto por `TraceObject.output_message_id`, cuyo `content_hash` conserva su propia regla de hash UTF-8. Un render ref ambiguo, un mensaje distinto o propiedades adicionales invalidan la reconstruccion.
...
40. `Answer.latest_answer_version_ref` debe resolver a un `AnswerVersion` del mismo `answer_id` y `organization_id`; no puede apuntar a una version de otra respuesta o tenant.
41. Tras confirmar una nueva version, `Answer.latest_answer_version_ref` debe apuntar a la version vigente con el mayor `answer_version` de esa respuesta. La actualizacion de version y puntero es atomica; no se permite un puntero atrasado o adelantado.
42. `Answer.response_outcome` debe ser exactamente igual a `response_outcome` del `AnswerVersion` resuelto por `latest_answer_version_ref`.
43. La regla 4 y las reglas 17 a 42 requieren validador custom, constraint de persistencia o transaccion de servicio; no pueden verificarse con cada JSON Schema aislado.
...
- La politica exige que `Answer.latest_answer_version_ref` resuelva a la version vigente de la misma respuesta y tenant, con `response_outcome` consistente.
