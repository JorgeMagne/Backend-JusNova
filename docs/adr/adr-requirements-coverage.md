...
| ADR-002 | Redis no es fuente de verdad. | ADR-002: Decision; `architecture-overview.md`. | Accepted | Fase 1 prepara compose/readiness; cache, locks o rate limiting se implementan cuando una funcionalidad aceptada los requiera, sin mover verdad transaccional fuera de PostgreSQL. | Perdida de datos transaccionales. |
...
| ADR-011 | Retencion/eliminacion. | ADR-011: Reglas aprobadas; `privacy-security-policy.md`; `document-security-policy.md`. | Accepted | Fase 1 implementa tombstone solo en superficies aceptadas/implementadas; el borrado documental completo se vuelve obligatorio al habilitar upload/storage/document processing y antes de market. | Datos retenidos sin politica. |
